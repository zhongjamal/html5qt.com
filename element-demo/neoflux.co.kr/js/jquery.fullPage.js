    ...
    $.fn.fullpage = function(options) {
        ...
        */
        function addVerticalNavigation(){
            $body.append('<div class="' + SECTION_NAV + '"><ul></ul></div>');
            ...
            for (var i = 0; i < $(SECTION_SEL).length; i++) {
                ...
                // var li = '<li><a href="#' + link + '"><span><img src="./images/icon/nav_tab.png" alt="" /></span></a>';
            ...
            $(SECTION_NAV_SEL).css('margin-top', '-' + ($(SECTION_NAV_SEL).height()/3) + 'px');

            //activating the current active section
            // 20160317 기본 네비 액티브
            // $(SECTION_NAV_SEL).find('li').eq($(SECTION_ACTIVE_SEL).index(SECTION_SEL)).find('a').addClass(ACTIVE);
            $(SECTION_NAV_SEL).find('li').eq($(SECTION_ACTIVE_SEL).index(SECTION_SEL)).find('a').addClass(ACTIVE).find('img').attr('src','./images/icon/nav_tab_on.png');
        }
        ...
        */
        ...
        function activateNavDots(name, sectionIndex){
            // 160411 NavDot position
            var move = sectionIndex*49;
            if (sectionIndex < 4){
                $('#dHead .navi-bar img').animate({marginTop:move}, 1600, 'easeInOutExpo');
            } else if (sectionIndex >= 4){
                $('#dHead .navi-bar img').animate({marginTop:147}, 1600, 'easeInOutExpo');
            }
            if(options.navigation){
                // 20160316 네비아이콘
                $(SECTION_NAV_SEL).find(ACTIVE_SEL).removeClass(ACTIVE);
                // $(SECTION_NAV_SEL).find(ACTIVE_SEL).removeClass(ACTIVE).find('img').attr('src','./images/icon/nav_tab.png');
                if(name){
                    $(SECTION_NAV_SEL).find('a[href="#' + name + '"]').addClass(ACTIVE);
                }else{
                    // 20160316 네비아이콘
                    $(SECTION_NAV_SEL).find('li').eq(sectionIndex).find('a').addClass(ACTIVE);
                    // $(SECTION_NAV_SEL).find('li').eq(sectionIndex).find('a').addClass(ACTIVE).find('span img').attr('src','./images/icon/nav_tab_on.png');
                }
            }
        }
